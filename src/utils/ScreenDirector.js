    ...
    for (const dictum_name in this.manifesto) {
      ...
      this.on(`${dictum_name}`, async function( dictum_name ){
        ...
        if(Array.isArray(dictum.logic)){     // Differentiate between singular functions and function arrays
          ...
          logic_count = 1;
          let logic_response = await dictum.logic();  // Wait on the response to this dictum's one logic algorithm... NOTE: Must be a valid thing to pass on to the _progress event.
...
class Screenplay{
  ...
  animate = ()=>{
    ...
    if (this.delta  > this.interval) {
      this.scene_assets.update( this.delta );
      this.scene_assets.direct( this.delta );
      this.scene_assets.render();
      this.scene_assets.ui_render();
